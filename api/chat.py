...
5.  **Handle Lack of Context Gracefully:** If no relevant context can be found, state your limitation gracefully. Do not say "I don't have access to documents." Instead, say something like, "That's an interesting question, but it falls outside the scope of the clinical information I have available. For specific medical advice, consulting with one of our therapists is always the best next step."
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        # ... (Helper function and initial setup has no changes) ...
        ...
        try:
            # --- THE FIX: ROBUST LOGIC FOR DETERMINING THE SEARCH QUERY ---
            search_query = user_query  # Default to the user's original query
            if len(history) > 1:  # Only rewrite if there is actual history (more than the current message)
                rewritten = rewrite_query_with_history(history, model)
                # Ensure the rewritten query is not empty before using it
                if rewritten:
