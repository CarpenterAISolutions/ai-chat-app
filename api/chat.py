...
async def handle_chat(chat_request: ChatRequest):
    ...
    try:
        ...
        if not all([gemini_api_key, pinecone_api_key]):
            raise HTTPException(status_code=500, detail="Server Configuration Error: One or more API keys are not set in the Vercel environment.")

        genai.configure(api_key=gemini_api_key)
        pc = Pinecone(api_key=pinecone_api_key)

        if pinecone_index_name not in pc.list_indexes().names():
            raise HTTPException(status_code=500, detail="Knowledge Base Error: The Pinecone index named 'physical-therapy-index' was not found.")

        index = pc.Index(pinecone_index_name)

    except Exception as e:
        # This will catch any error during initialization and report it clearly.
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Invalid request format from frontend: {str(e)}")
    ...
    try:
        ...
        print(f"RAG Search Warning: Could not retrieve context. Error: {str(e)}")
